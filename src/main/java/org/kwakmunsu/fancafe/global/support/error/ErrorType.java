...
public enum ErrorType {
    ...
    MEMBER_CANNOT_WRITE_POST     (HttpStatus.FORBIDDEN, "게시글 작성 권한이 없습니다.", LogLevel.WARN),
    ...
    // POST
    POST_INVALID_TITLE   (HttpStatus.BAD_REQUEST, "게시글 제목은 1자 이상 200자 이하여야 합니다.", LogLevel.INFO),
    POST_INVALID_CONTENT (HttpStatus.BAD_REQUEST, "게시글 내용은 50,000자를 초과할 수 없습니다.", LogLevel.INFO),
    POST_CANNOT_PUBLISH  (HttpStatus.BAD_REQUEST, "임시저장 또는 비공개 상태의 게시글만 발행할 수 있습니다.", LogLevel.WARN),
    POST_CANNOT_HIDE     (HttpStatus.BAD_REQUEST, "발행된 게시글만 비공개로 전환할 수 있습니다.", LogLevel.WARN),
    POST_NOT_FOUND       (HttpStatus.NOT_FOUND, "존재하지 않는 게시글입니다.", LogLevel.INFO),

    // CATEGORY
    CATEGORY_NOT_FOUND (HttpStatus.NOT_FOUND, "존재하지 않는 카테고리입니다.", LogLevel.INFO),
